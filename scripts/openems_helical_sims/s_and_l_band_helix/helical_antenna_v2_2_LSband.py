...
# -*- coding: utf-8 -*-
"""
 Dual-Band Helical Antenna Simulation
 ...
 (c) 2026 Mete Han Keskin <mete@kestech.net>
      2026: extended to dual-band concentric helix configuration
...
Sim_Path = os.path.join(tempfile.gettempdir(), 'Helical_Anten_doublebnd')
...
# Simulation box based on the larger (lower-frequency) wavelength
...
max_res = np.floor(C0 / (f_center + fc) / unit / 20) # cell size: lambda/20
...
# Wide sweep to observe both bands (note: data quality degrades near excitation edges)
...
nf2ff_res = nf2ff.CalcNF2FF(Sim_Path, f_center, theta, phi, read_cached=False, verbose=True)
